...
def test_confirm_entry_exit(client):
    ...
    assert 'Bienvenido+Alice' in r1.headers['location'] or 'Entrada+registrada+para+Alice' in r1.headers['location']
...
def test_multiple_entry_exit_cycles(client):
    ...
    for i in range(2):
        r1 = client.get(f'/confirmar?id={alice_id}', follow_redirects=False)
        assert r1.status_code == 303
        assert 'Entrada+registrada+para+Alice' in r1.headers['location'] or 'Bienvenido+Alice' in r1.headers['location']
